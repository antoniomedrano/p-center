...
#   combinations = itertools.combinations(range(n),5) # generate iterator containing
#                                           # all possible combinations of 3 columns
#   N = n*(n-1)*(n-2)*(n-3)*(n-4)//120 # number of combinations (length of combinations cannot be
...
#       current_comb = np.fromiter(combinations,dtype='i,i,i,i,i',count=counts)\
#                       .view(('i',5))
...
#   def your_func(A):
#     n=A.shape[0]
#     save_rows=np.zeros(5,dtype=np.uint64)
#     global_best=np.inf
#     for i in range(n-4):
...
#                         local_best = max_min_5(A[i,:], A[j,:], A[k,:], A[l,:], A[m,:])
...
#   print(total_time)
  ...
  def your_func_parallel(A):
    ...
    rows=np.empty((5),dtype=np.uint64)
    save_rows=np.empty((n,5),dtype=np.uint64)
    global_best_Temp=np.empty((n),dtype=A.dtype)
    global_best_Temp[:]=np.inf

    for i in range(n-4):
        for j in nb.prange(i+1, n-3):
            row_1=0
            row_2=0
            row_3=0
            row_4=0
