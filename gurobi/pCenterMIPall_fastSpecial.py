...
def Run_pCenter():
    ...
    for i in range(3, numSites):
        ...
        if i == 387:
            diff, C = updateCoverCoefficeints(distMatrix, SDmin+.1, C)
        else:
            diff, C = updateCoverCoefficeints(distMatrix, SDmin+.000001, C)
        #tried removing one of the zeros from the SDmin sum for u574.tsp, perhaps try 2
        #original code has 5 zeros after decimal
