...
def Run_pCenterLSCP():
    ...
    for k in range(1,len(sqDistances)):
        ...
        if (p == 5):
            p = 4
            if numSites > 111:
                SDsquared, rows = brute.nbParallel4(sqDistMatrix, numSites)
            elif numSites > 60:
                SDsquared, rows = brute.nbSerial4(sqDistMatrix, numSites)
            else:
                SDsquared, rows = brute.chunk4(sqDistMatrix, numSites)
